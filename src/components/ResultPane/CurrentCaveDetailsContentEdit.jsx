        ...
        {rows.map((row, index) => (
          <Box key={index} sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            <TextField
              ...
              value={row.lang}
              onChange={(e) => updateAt(index, { lang: e.target.value })}
            >
              {languages
                .filter(l => l.code === row.lang || !usedLangs.includes(l.code))
                .map(l => (
                  <MenuItem key={l.code} value={l.code}>{l.eng}</MenuItem>
                ))}
            </TextField>
    ...
    <Box>
      ...
      {value && (
        <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, p: 1, mt: 0.5 }}>
          <Typography variant="caption" color="text.secondary">{t('preview')}</Typography>
...
export default function CurrentCaveDetailsContentEdit({ cave }) {
  ...
  return (
    <Box className="oc-result-pane--content" sx={{ display: 'flex', flexDirection: 'column', gap: 2, p: 'var(--oc-pane-padding-inline)' }}>
      <TextField label={t('name')} fullWidth required {...field('name')} />

      <RepeatableTextField
        label={t('aka')}
        values={form.aka}
        onChange={(aka) => setForm(f => ({ ...f, aka }))}
        addLabel={t('addAka')}
        removeLabel={t('removeAka')}
      />

      <NameTranslationsField
        label={t('nameTranslations')}
        rows={form.nameTranslations}
        languages={languages}
        onChange={(nameTranslations) => setForm(f => ({ ...f, nameTranslations }))}
        addLabel={t('addNameTranslation')}
        removeLabel={t('removeNameTranslation')}
        languageLabel={t('nameTranslationLanguage')}
        valueLabel={t('nameTranslationValue')}
      />
      ...
      <CoordinateField
        ...
        onChange={({ longitude, latitude }) => setForm(f => ({ ...f, longitude, latitude }))}
      />
      <CoordinateField
        field="entrance"
        label={t('entrance')}
        longitude={form.entranceLongitude}
        latitude={form.entranceLatitude}
        onChange={({ longitude, latitude }) => setForm(f => ({ ...f, entranceLongitude: longitude, entranceLatitude: latitude }))}
      />
      ...
      <TextField select label={t('access')} fullWidth {...field('access')}>
        <MenuItem value="">{t('none')}</MenuItem>
        {accesses.map(a => (
          <MenuItem key={a.id} value={a.id}>{a.name}</MenuItem>
        ))}
      </TextField>
      <TextField select label={t('accessibility')} fullWidth {...field('accessibility')}>
        <MenuItem value="">{t('none')}</MenuItem>
      ...
      <Box sx={{ display: 'flex', gap: 1, position: 'sticky', bottom: 0, bgcolor: 'background.paper', py: 1 }}>
        <Button variant="contained" onClick={handleSave} disabled={saving || !form.name}>{t('save')}</Button>
        <Button onClick={exitEditMode} disabled={saving}>{t('cancel')}</Button>
      </Box>
    </Box>
  )
}
