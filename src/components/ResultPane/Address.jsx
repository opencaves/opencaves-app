...
export default function Address({ latitude, longitude }) {
  ...
  const { data, error, isLoading } = useSWR(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=${process.env.VITE_GOOGLE_GEOCODING_API_KEY}&language=${i18n.resolvedLanguage}&result_type=${resultTypes.join('|')}`, fetcher)
